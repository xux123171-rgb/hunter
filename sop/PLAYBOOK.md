...
**A8 回显三查（testfire 教训：SQLi 无 diff ≠ 回显安全，差点漏报反射XSS）**：每个有回显的参数必须三项独立测完才许判死——① 注入（布尔/延时）② **XSS canary 活体回显（`<img src=x onerror=9>` 进页面查 DOM 是否原样渲染+有无 CSP/转义）** ③ 跳转/URL 参数（redirect/url 回显进 href=）。只测了①就写 A8 判死=流程违规。
...
### B. 红线（平台授权也不打）
内网段（127./10./172.16-31./192.168. 不可路由不探不报拖）、横向移动/跳板、拖全库、DoS/并发轰炸、社工钓鱼、任何破坏性写入。授权边界只到公网资产。**内网正确打法=梯子 L7：公网资产当代理，找 SSRF 位回显内网数据（这是合规高危实锤），不是从本机直连内网。**
### A'. 移动端线（大厂 SRC 富矿：App/小程序接口常比 Web 面裸）
授权含 App 接口（多数 SRC 写"官方 App"）时：① 官网/应用宝下载 apk（几百字实验签名再下），`apktool d`/jadx 反编译；② 挖 `config`/`BuildConfig`/RN bundle 里的 **API 基址+AK/SK+硬编码 token+内网域名**（App 内置 IP 还是 L5 找源站的黄金输入）；③ 小程序：反编译包同法（wxapkg）；④ 拿到签名方案后**只用 1 组自造参数**测接口（不重放他人流量、不爬全量）；⑤ App 接口往往不走 Web WAF——web 判死的站，同一业务的 `/app-api/` 可能裸奔。此线工具暂手工（jadx/apktool），流程已入 SOP；`tools/` 后续可补 apk-secret-scan 腿。
...
**判死复审梯子（出洞率保险丝）**：三件套只证"扫过"，不证"绕尽"。任何格写判死前，按 `references/death-escalation-ladder.md`（`hunter_brain("ladder")`）从 L0 往上爬：L1 路径变体 → L2 方法/协议(chunked/H2) → L3 载荷变形(双编码/HPP) → L4 头伪装 → L5 源站+历史资产+鬼资产重试 → L6 墙后伴生面(prometheus/nacos/knife4j/SPA 未鉴权预览接口) → L7 内网靶标 SSRF（泄露的内网IP是靶标不是禁区：公网资产替你请求内网，回显=高危实锤）。每级 1-2 发、全梯 ≤15 发/面；梯级结果写进判死证据（`L0✓ L1✓…`）。**"被 WAF 拦"不等于证据穷尽——没爬完梯子就没有判死资格。**
