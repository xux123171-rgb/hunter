...
| 能力 | Bug-Bounty-recon | bugkit | ReconOPS | **hunter** |
|---|---|---|---|---|
| 定位 | 管道编排 | 18 模块 recon CLI | 纯 recon 方法论 | **脑子+腿+洞型全谱系** |
| 作战 SOP/阶段门 | ❌ | ❌ | 📄 仅 recon 层 | ✅ 6 阶段+阶段门 |
| **判死纪律**（何时停、证据制） | ❌ | ❌ | ❌ | ✅ 判死必须写证据，3 手法全拦即停 |
| 实锤标准（什么算洞） | ❌ | ❌ | ❌ | ✅ 真数据/真越权/真回读，success/报错不算 |
| 合规深度封顶（打几发停） | ❌ | ✅ --authorized 门 | ❌ | ✅ 每洞型封顶表+限速+payload 克制 |
| 越权双账号通道 | ❌ | ❌ | ❌ | ✅ mailacct.py 自研收件 |
| 存储 XSS 三态判定 | ❌ | ❌ | ❌ | ✅ xssprobe 元素级 canary |
| 报告产出（平台字段级） | ❌ | ❌ | ❌ | ✅ 补天/盒子可复制模板 |
| 攻击状态机（矩阵清空才准收工） | ❌ | ❌ | ❌ | ✅ matrix.md 每格终态制 |

差异化一句话：同类项目解决"面怎么扫"，hunter 解决**"扫完怎么判、判完怎么停、停完怎么报"**——把白帽子最缺的纪律（实锤/判死/封顶）变成带状态机的代码流程。
